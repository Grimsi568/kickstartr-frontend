...
const BundleCard = ({ bundle }: BundleCardProps) => (
  ...
  >
    ...
    <div className="relative z-10 flex-1 flex flex-col px-4 sm:px-6 lg:px-7 pb-4">
      <p className="text-sm sm:text-base text-neutral-200 mb-4 font-light">{bundle.description}</p>
      <div className="flex flex-wrap gap-2 mb-4 sm:mb-6">
        {bundle.templates?.length
          ? bundle.templates.map(template => (
              <span key={template.id} className="px-2 sm:px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-cyan-400/40 via-blue-400/30 to-cyan-600/40 text-cyan-100 border border-cyan-300 shadow-cyan-400/10 shadow-sm">
                {template.name}
              </span>
            ))
      ...
      <div className="mt-auto flex justify-end">
        <a
          ...
          className="btn btn-sm btn-primary font-mono tracking-wide bg-cyan-400/80 border-cyan-400 text-white hover:bg-cyan-500/90 hover:border-cyan-300 transition-all text-xs sm:text-sm px-3 sm:px-4 py-2"
        >
          View Details
        </a>
      </div>
