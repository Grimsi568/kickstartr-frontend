...
const UserInfoCard: React.FC<Props> = ({ displayName, email, id, createdUtc }) => (
  <div className="card p-8 mb-8 flex flex-col items-center bg-gradient-to-br from-cyan-900/40 to-cyan-800/10 border border-cyan-700 shadow-lg rounded-xl">
    ...
    <div className="w-full flex flex-col gap-2 text-sm">
      <div className="flex justify-between">
        ...
        <span className="font-mono text-neutral-300">{createdUtc ? new Date(createdUtc).toLocaleDateString() : '-'}</span>
      </div>
    </div>
  </div>
)
